...
OPTIMIZE := -O0
WARNINGS := -Wall 
DEFS     := -DCOMPILE_FOR_OSDCLIENT
EXTRA_CFLAGS := -g -std=c++0x
LIBS := -L. -L -lpthread -llog4cpp -lcurl
INC_DIR   = $(BUILD_HOME)/include $(BUILD_HOME)/3rdparty/mxml/include
SRC_DIR   = $(BUILD_HOME)3rdparty/mxml/src $(BUILD_HOME)/src
OBJ_DIR   = ./obj
EXTRA_SRC = 
EXCLUDE_FILES = 

SUFFIX       = c cpp cc cxx
TARGET       := $(BIN_HOME)/downc
TARGET_TYPE  := bin
...
CFLAGS       = $(EXTRA_CFLAGS) $(WARNINGS) $(OPTIMIZE) $(DEFS) ${LIBS}
GCCFLAGS       = $(OPTIMIZE) $(DEFS) ${LIBS}
